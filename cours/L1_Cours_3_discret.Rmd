...
<div class="my-footer"><span>ENSG - L1 `r annee` : Statistiques appliquées en cartographie- Florian Bayer</span></div> 
La longueur de la variable visuelle valeur ne permet pas à l'œil d'associer un niveau de gris à une seule valeur. Seule la notion d'ordre est transmise. Il est nécessaire de réduire le nombre de niveaux de gris pour y associer une plage de valeur (de 0 à 7, puis de 7 à 15 par exemple).
...
Dans le cas contraire, vous risquez d’avoir une carte n’apportant que très peu d'informations, car la discrétisation sera mal adaptée au message cartographique.
...
- Très facile à réaliser.
- Facilement compréhensible par le lecteur.
- Permet la comparaison dans le temps d’un même phénomène (il faut discrétiser sur une même série comprenant toutes les dates). Les bornes de classe seront les mêmes entre les deux dates.
...
- Garder les informations de la distribution statistique.

En règle générale, le nombre de classes va de **4 à 7**
- En dessous, l’information spatiale sera trop faible.
- Au-delà, la carte sera trop complexe à comprendre : trop d’informations visuelles, la longueur de la variable visuelle valeur ne permet pas à votre œil d'associer les différents niveaux de gris de la carte avec ceux de la légende.
...
- Soit comparer des données de même nature. On privilégiera la comparaison **absolue.** : on compare les unités de mesure. Les bornes de classes doivent donc être identiques.
...
- En ajustant le minimum (11 vs 24) et en ajoutant une classe supplémentaire pour 2009.
]
...
- mais vous ne pourrez pas comparer votre carte dans le temps car les bornes des classes des deux cartes seront différentes.
]
...
.center-img[
```{r echo=FALSE, out.width="80%"}
knitr::include_graphics("https://i.imgur.com/TVyYDpt.png")
```
]
