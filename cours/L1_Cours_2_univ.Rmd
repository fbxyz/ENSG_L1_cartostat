...
On représente généralement les **données qualitatives discrètes / exhaustives** à l'aide d'un diagramme en bâton. Il faut réaliser un **dénombrage** à l'aide d'un tableau de contingence. 
...
L’idéal est de créer des **classes d’amplitudes égales** (les classes vont de 10 en 10 ou de 20 en 20). Le graphique ressemble alors à un diagramme en bâton, sans discontinuité.

Si les classes ne sont pas de même amplitude (de 10 à 20, puis de 20 à 120), la surface de la colonne (et non sa hauteur) doit être proportionnelle au nombre d’individus représentés. Dans la cas contraire, l’impression visuelle ne sera pas conforme à la réalité. Peu de logiciels proposent cette fonctionnalité, il est donc préférable de faire des classes d'amplitudes égales.
...
.pull-left[
- La série est **<span style="color:#f8766d">unimodale</span>** : les données sont concentrées autour d’un seul point.
- La série est **<span style="color:#00bfc4">multimodale</span>** : les données sont concentrées autour d’un mode principal et un ou plusieurs modes secondaires.
]
...
Cette approche graphique permet déjà de poser des hypothèses sur nos données. Le castor 3 a une température normale (autour de 37°C) alors que le castor 4 semble en hypothermie, à l'inverse du castor 5 qui pourrait faire une réaction inflammatoire.
...
Les principaux outils de l’analyse univariée peuvent être classés en 2 familles :
...
A noter que la classe modale est la seule valeur centrale utilisable pour les données qualitatives, après dénombrement (cf. diagramme en bâton)
...
Cet indicateur est facile à comprendre mais n’a pas les propriétés statistiques intéressantes. On préfère donc utiliser l'écart-type.   
]
...
- mais calculé avec le théorème de Pythagore et non plus avec la valeur absolue 
...
.pull-right[
La comparaison des distributions et de leur forme est très facile avec le box plot (ici en 1,5x l’intervalle interquartile, on le reconnaît aux outliers -valeurs hors norme- en forme de points).  
...
La forme d'une série peut être évaluée graphiquement, mais cette approche reste subjective. Certains se contentent de comparer la position de la moyenne et de la médiane, mais sans réel formalisme.
...
Il existe également des tests statistiques pour déterminer si une série suit une loi particulière. On utilise généralement un test de Shapiro-Wilk. Un test de normalité calculable à la main est celui de Jarque-Bera. Il se base sur le coefficient d'asymétrie et le kurtosis.
...
La statistique JB se teste via une loi du $\chi^2$ à deux degrés de liberté. On pose H0 les données suivent une loi normale et H1 les données ne suivent pas une loi normale. Si les données suivent une loi normale, le résultat du test s'approche alors de 0 et on ne rejette pas Ho au seuil $\alpha$. 
...
Pour les graphiques :
- La représentation des données qualitatives se fait avec un diagramme en bâton. Il faut souvent *dénombrer* les données à l'aide d'un tableau de contingence. 
...
- A l'aide de l'intervalle interquartile. 
...
Attention, faire un graphique et donner le résultat d’un calcul n’est pas une plus-value : un logiciel de statistique peut le faire automatiquement. 
